...
try:
    from plugin_system import PluginManager, HotReloadManager
...
from mlx_lm.generate import BatchGenerator, generation_stream, stream_generate
        ...
        while not self._stop:
            ...
            if request is not None:
                ...
                if (
                    ...
                    _has_thinking = getattr(tokenizer, 'has_thinking', False)
                    try:
                        _think_start_id = tokenizer.think_start_id if _has_thinking else -1
                    except (TypeError, AttributeError):
                        _think_start_id = -1
                    try:
                        _think_end_id = tokenizer.think_end_id if _has_thinking else -1
                    except (TypeError, AttributeError):
                        _think_end_id = -1
                    try:
                        _think_end = tokenizer.think_end if _has_thinking else ""
                    except (TypeError, AttributeError):
                        _think_end = ""
            ...
            # No request so serve from the current batch
                ...
                for _ in self._time_budget:
                    prompt_responses, responses = batch_generator.next()
                    if not prompt_responses and not responses:
                        break

                    for r in prompt_responses:
                        if r.uid in batch_results:
                            batch_results[r.uid]["rqueue"].put(r.progress)
...
class APIHandler(BaseHTTPRequestHandler):
        ...
        def parse_tools(tool_calls):
            ...
            for tool_text in tool_calls:
                ...
                try:
                    ...
                    # Fallback for Qwen-style XML tags
                    import re
                    func_match = re.search(r'<function=(.*?)>(.*?)</function>', tool_text, re.DOTALL)
                    if func_match:
                        func_name = func_match.group(1).strip()
                        params_body = func_match.group(2).strip()
                        params = {}
                        for p_match in re.finditer(r'<parameter=(.*?)>(.*?)</parameter>', params_body, re.DOTALL):
                        ...
                        logging.info(f"Successfully parsed XML tool call for function: {func_name}")
                    else:
                        logging.warning(f"Failed to parse tool call: {tool_text}")
                        continue
                
                # Success log for the whole process
                logging.info(f"Generated {len(parsed)} tool calls successfully.")
    ...
    def do_GET(self):
        ...
        """
        if self.path.startswith("/v1/plugins"):
            self.handle_plugins_request()
...
def get_plugin_manager() -> Optional[PluginManager]:
    ...
    if PLUGIN_SYSTEM_AVAILABLE and _plugin_manager is None:
        _plugin_manager = PluginManager(
            proxy_port=8080,  # Server port
            api_port=8081     # API port (farklı port)
        )
...
def get_hot_reload_manager() -> Optional[HotReloadManager]:
    ...
    if PLUGIN_SYSTEM_AVAILABLE and _hot_reload_manager is None:
        pm = get_plugin_manager()
        if pm:
            _hot_reload_manager = HotReloadManager(pm)
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable plugin system for hot-reload and dynamic model loading",
    )
    parser.add_argument(
        "--host",
        type=str,
    ...
    if args.enable_plugin_system and PLUGIN_SYSTEM_AVAILABLE:
        pm = get_plugin_manager()
        if pm:
            pm.start_api_server()
            print(f"[*] Plugin API running on port 8081")
